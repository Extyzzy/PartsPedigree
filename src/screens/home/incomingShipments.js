...
type Props = {
  shipments: Array<EventModel>;
}

@branch({
  shipments: ['shipments']
})
export class IncomingShipments extends ListingBase<Props> {
    static contextTypes = {
    ...
    async getData() {
        ...
        this.cursorNext = await BackendApi.getIncomingShipmentsList(cursorNext);
    ...
    renderLogo(event) {
      if ( event.fromOrganization !== '') {
        return (
          <ServerImage
            style={{width: 70, height: 50, marginRight: 10}}
            uri={event.fromOrganization && event.fromOrganization.logoPath}
          />
        )
      } else {
        return(
            ...
            <View style={[commonStyle.rowBetween, commonStyle.indent(ScreenDetector.isPhone() ? 5 : 15)]}>
              <View style={[commonStyle.rowBetween]}>
                <View style={{width: 150}}>
                  ...
                  <Text style={[style.shipmentsTitleText]}>From: {item.fromOrganization && item.fromOrganization.name}</Text>
                </View>
